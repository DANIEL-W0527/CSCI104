...
void ProductSectionParser::addProductParser(ProductParser* p) { prodParsers_.insert(make_pair(p->categoryID(), p)); }

bool ProductSectionParser::parse(std::istream& is, DataStore& ds, int& lineno, std::string& errorMsg) {
    string line;
...
void ProductSectionParser::reportItemsRead(std::ostream& os) { os << "Read " << numRead_ << " products" << endl; }
...
void UserSectionParser::reportItemsRead(std::ostream& os) { os << "Read " << numRead_ << " users" << endl; }
